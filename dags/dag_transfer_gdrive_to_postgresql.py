...
def __authenticate():
    ...
    package_to_install = ["google-api-python-client", "google-auth-httplib2", "google-auth", "google-auth-oauthlib"]
    __install_packages(package_to_install)

    from google.oauth2.credentials import Credentials

    creds = None
    # get token from airflow variable
    token_path = Variable.get(key="dag_transfer_gdrive_postgresql_prod")
...
def __get_folders_id(service, folder_name):
    query = f"shareWithMe=true and name='{folder_name}'"
    results = (
        service.files()
...
def _check_file_if_exists(ti):
    from googleapiclient.discovery import build
    from googleapiclient.errors import HttpError

    try:
        creds = __authenticate()
        service = build("drive", "v", credentials=creds, cache_discovery=False)

        dataset_folder_name = "dataset"
        dataset_folders = __get_folders_id(service, dataset_folder_name)

        if dataset_folders == []:
            print(f"The '{dataset_folder_name}' files does not exist in 'Share with me' in folder 'dataset'.")
            return "end"
        
        # get id of 'dataset' folder
        dataset_folder_id = dataset_folders[0]['id']
        print(f"The '{dataset_folder_name}' folder exists in 'Share with me'.")
        ...
        print(f"An error accured : {error}")

def __compose_value(row):
    return f"{row['IP']}_{row['UserAgent']}_{row['Country']}_{row['Language']}_{row['Interest']}"

def _process_csv(ti):
    import csv

    # pull xcom
    dataset_files = ti.xcom_pull(key="dataset_files", task_ids="check_files_if_exist")
    ...
    for file in dataset_files:
        ...
        with open(input_file, 'r', newline='') as csvfile:
            # open the output CSV file for witing
            list_processed_name = []
            with open(output_file, 'w', newline='') as output_csvfile:
                ...
                for row in reader:
                    ...
                    new_field_value = __compose_value(row)

                    # Add the new field to the row
                    row['id'] = new_field_value

                    # Write the modified row to the outputCSV file
                    writer.writerow(row)
...
def __create_ddl():
    ...
    # Install psycopg2
    package_to_install = ('psycopg2-binary')
    __install_packages(package_to_install)
    import psycopg2

    ddl = """
    CREATE TABLE IF NOT EXIST public.network_data (
        id TEXT PRIMARY KEY,
         IP VARCHAR(15),
        UserAgent TEXT,
        Country VARCHAR(255),
...
def _upset_data(ti):
    ...
    db_params = {
        ...
        'user': 'airflow',
        'password': 'airflow',
        'host': '172.21.0.3', # Change this to your PostgreSQL server address
        'port': '5432',    # Change this to your PostgreSQL server port
    }

    # CSV file path
    # pull xcom 
    list_processed_name = ti.xcom_pull(key="listprocessed_name", task_ids="process_csv")
...
with DAG(
    ...
    check_file_if_exist = PythonOperator(
        ...
        python_callable=_check_file_if_exists,
    ...
    upset_data = PythonOperator(
        task_id='upset_data',
        python_callable=_upset_data,
        execution_timeout=timedelta(minutes=5),
        retries=2,
        retry_delay=timedelta(minutes=5),
    )
    ...
    process_csv >> create_ddl >> upset_data >> end_task